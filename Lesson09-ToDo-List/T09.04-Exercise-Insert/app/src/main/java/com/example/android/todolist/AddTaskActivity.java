...
public class AddTaskActivity extends AppCompatActivity {
    ...
    public void onClickAddTask(View view) {
        ...
        contentValues.put(TaskContract.TaskEntry.COLUMN_DESCRIPTION, inputText);
        contentValues.put(TaskContract.TaskEntry.COLUMN_PRIORITY, mPriority);

        // TODO (7) Insert new task data via a ContentResolver
        ContentResolver contentResolver = getContentResolver();
        Uri insertedRow = contentResolver.insert(TaskContract.TaskEntry.CONTENT_URI, contentValues);

        // TODO (8) Display the URI that's returned with a Toast
        Toast.makeText(getBaseContext(), insertedRow.toString(), Toast.LENGTH_LONG).show();
